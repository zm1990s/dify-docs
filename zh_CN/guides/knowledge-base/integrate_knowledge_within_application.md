...
3. **如何使用多路召回中的 Rerank 设置？**

我们将举个简单例子说明“多路召回”的 Rerank 设置，帮助你了解如何选择最适合的内容检索效果：

多路召回中的 “权重设置” 可以满足低成本兼粗略的检索方案，这是一种在不引入外部 Rerank 模型的情况下，通过语义或关键词检索，实现文本的精准匹配。

例如，当前应用关联了多个描述不同电商产品参数的知识库，用户需要通过具体的序列号查询对应的产品参数，日常查询频率较高，此时就可以调整 “权重设置”，提高关键词精确匹配的重要性和优先级。

如果你想要提高检索的精确度，此时就需要应用 “Rerank 模型” 帮助你使用不同的检索技术来获得更好的召回效果。当然，这需要你额外花费 rerank 模型的费用，与之而来的回答效果也更加精准。

> 扩展阅读：[《Re-ranking》](https://docs.dify.ai/learn-more/extended-reading/retrieval-augment/rerank)

例如，当前应用的关联了 5 个和科研期刊有关的知识库，内容专业性较高且无法通过简单的关键词匹配找到准确答案，并且客户提出的问题也更为复杂，对于结果的要求也更为精准，可以接受由高频查询所带来的花费，会更加推荐和方法。
