...
该设置无需配置外部 Rerank 模型，重排序内容**无需额外花费**，但依然可以通过语义或关键词配置，确保内容匹配质量。你可以选择固定权重比例的 **“关键词 & 语义优先”** 配置，或通过不断调试，自定义权重比例。

- **关键词 & 语义优先**
  
  **关键词检索** 通过用户输入的信息文本在知识库全文匹配，适用于用户知道确切的信息或术语的场景。该方法所消耗的计算资源较低，适合在大量文档的知识库内快速检索。
