...
WebアプリのURLです。ファイルプレビューのためと**フロントエンド**にダウンロード用のURLを表示ます、またはマルチモデル入力として使用されます。空の場合は、アプリと同じドメインに黙認します。例：`https://udify.app/`
...
デフォルト値は60000 (milliseconds). 一部のプロセスが進行中にタイムアウトの原因で全部のサービスが利用できなくなるのを防ぐために、テクストの生成やワークフロー進行中にタイムアウトの時間を指定するために使用されます。
...
WebApp Url，**フロントエンド** API バックエンド アドレスを宣言するために使用されます、空の場合は同じドメインになります。范例：`https://udify.app/`
