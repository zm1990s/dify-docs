...
Dify supports file uploads in both [ChatFlow](key-concept.md#chatflow-and-workflow) and [WorkFlow](key-concept.md#chatflow-and-workflow) type applications, processing them through [variables](variables.md) for LLMs. Application developers can refer to the following methods to enable file upload functionality:
...
1.  Click the **"Features"** button in the upper right corner of the Chatflow application to add more functionality to the application.
...
<figure><img src="../../../img/file-upload-1.png" alt=""><figcaption><p>File Upload Feature</p></figcaption></figure>

Enabling this feature does not grant LLMs the ability to directly read files. A [**Document Extractor**](node/doc-extractor.md) is still needed to parse documents into text for LLM comprehension.
...
- For video and other file types, there are currently no corresponding extractors. Application developers need to integrate [external tools](../tools/advanced-tool-integration.md) for processing.

2. Add a [Document Extractor](node/doc-extractor.md) node, and select the `sys.files` variable in the input variables.
...
<figure><img src="../../../img/file-upload-2.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="../../../img/file-upload-3.png" alt=""><figcaption></figcaption></figure>
...
Add input fields in the application's ["Start"](node/start.md) node, choosing either **"Single File"** or **"File List"** as the field type for the variable.
...
After uploading, files are stored in single file variables, which LLMs cannot directly read. Therefore, a [**"Document Extractor"**](node/doc-extractor.md) node needs to be added first to extract content from uploaded document files and send it to the LLM node for information processing.

Use the file variable from the "Start" node as the input variable for the **"Document Extractor"** node.

<figure><img src="../../.gitbook/assets/截屏2024-10-12 15.45.45.png" alt=""><figcaption><p>Add Input Variable</p></figcaption></figure>

Fill in the output variable of the "Document Extractor" node in the system prompt of the LLM node.

<figure><img src="../../.gitbook/assets/image (376).png" alt=""><figcaption><p>Paste System Prompt</p></figcaption></figure>
